...
Instance.prototype.getVmId = function(param, res, callback) {
    var self = this;
    console.log("Teste", param);
    var idvm= {};
    idvm.params.id = param._id;
    console.log("Teste2", idvm);
    vminstance.statusInstance(idvm, function() {
        self.InstanceDB.findOne(param, function(err, result) {
            if (err) {
                return res.json({
                    status: "ERROR",
                    stdout: "Error to load exists instances."
