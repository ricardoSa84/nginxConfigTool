    ...
    execCommandRemote: function(ipInstance, command, res) {
        ssh.connect({
            ...
            ssh.execCommand(command).then(function(output) {
            	...
            	if (output.stderr.toString().indexOf("syntax is ok") > -1) {
            		console.log("B");
            	}
            	if (output.stderr.toString().indexOf("test is successful") > -1) {
            		console.log("C");
            	}
            	if ((output.stderr.toString().indexOf("nginx: [warn] conflicting server name") > -1) && (output.stderr.toString().indexOf("ignored") > -1)){
            		console.log("D & E");res.send({
