    ...
    execCommandRemote: function(ipInstance, command, res) {
        ssh.connect({
            ...
            ssh.execCommand(command).then(function(output) {
            	console.log(output, output.stderr.toString().indexOf("ignored") > -1, output.stderr.toString().indexOf("syntax is ok") > -1, output.stderr.toString().indexOf("test is successful") > -1, output.stderr.toString().indexOf("nginx: [warn] conflicting server name") > -1, output.stderr.toString().indexOf("ignored") > -1);
                if ((output.stderr.toString().indexOf("ignored") > -1 && output.stderr.toString().indexOf("syntax is ok") > -1 && output.stderr.toString().indexOf("test is successful") > -1) || (output.stderr.toString().indexOf("nginx: [warn] conflicting server name") > -1 && output.stderr.toString().indexOf("ignored") > -1)){
                    return res.send({
