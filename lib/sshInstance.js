        ...
        ssh.connect({
            ...
            ssh.execCommand(command).then(function(output) {
                if ((output.stderr.toString().indexOf("ignored") > -1 && output.stderr.toString().indexOf("syntax is ok") > -1 && output.stderr.toString().indexOf("test is successful") > -1) || 
                	(output.stderr.toString().indexOf("syntax is ok") > -1 && output.stderr.toString().indexOf("test is successful") > -1) || 
                	(output.stderr.toString().indexOf("\[warn\]") > -1 && output.stderr.toString().indexOf("ignored") > -1)) {
                    return res.send({
