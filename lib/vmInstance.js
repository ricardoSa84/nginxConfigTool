...
module.exports.createInstance = function(req, res) {
    ...
    // var scriptBase64 = new Buffer("usermod --password $(echo 123qwe | openssl passwd -1 -stdin) root").toString('base64');
    ...
    var vmParams = 'MEMORY="' + ((data.templateRam * 1024) || 1024) +
        ...
        'SSH_PUBLIC_KEY="' + fs.readFileSync(config.sshKeyRoot).toString() + '", START_SCRIPT_BASE64 ="' + scriptBase64 + '"]\n';
    console.log("ParamsVm", vmParams);
    var vmParams2 = 'CONTEXT=[' +
        'SET_HOSTNAME="' + data.instanceName + '", ' +
        'NETWORK="' + 'YES' + '", ' +
        'NETWORK_UNAME="' + config.openNebulaparams.one.network + '", ' +
        'SSH_PUBLIC_KEY="' + fs.readFileSync(config.sshKeyRoot).toString() + '", ' +
    ...
    console.log("ParamsVm2", vmParams2);

    template.instantiate(data.instanceName, undefined, vmParams2, function(err, vm) {
        // console.log(err, vm);
        if (err) {
