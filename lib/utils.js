...
module.exports = {
    ...
    processReceivedJson: function(ipInstance, json, result, local, res) {
        ...
        try {
            if (local === "local") {
                saveFile(ipInstance, '/etc/nginx/conf.d/1000-' + json.servername + '-' + json.port + '.conf', "", "Server", local, result, res);
                ...
                cp.execSync('rm -rf $(pwd)/temp && mkdir $(pwd)/temp');

                saveFile(ipInstance, './temp/1000-' + json.servername + '-' + json.port + '.conf', '/etc/nginx/conf.d/1000-' + json.servername + '-' + json.port + '.conf', "Server", local, result, res);

                // fs.writeFile('./temp/1000-' + json.servername + '-' + json.port + '.conf', content, function(err) {
                //     if (err) {
                ...
                //         sendRemote.writeFileRemote(ipInstance, './temp/1000-' + json.servername + '-' + json.port + '.conf', '/etc/nginx/conf.d/1000-' + json.servername + '-' + json.port + '.conf', result, res);
                //     }
    ...
    processReceivedJsonUpstream: function(ipInstance, upsteams, result, local, res) {
        ...
        try {
            if (local === "local") {
                ...
                saveFile(ipInstance, '/etc/nginx/conf.d/0000-' + upsteams.name + '-upstreams.conf', "", "Upstream", local, result, res);
                ...
                saveFile(ipInstance, './temp/0000-' + upsteams.name + '-upstreams.conf', '/etc/nginx/conf.d/0000-' + upsteams.name + '-upstreams.conf', "Upstream", local, result, res);
                ...
                //         sendRemote.writeFileRemote(ipInstance, './temp/0000-' + upsteams.name + '-upstreams.conf', '/etc/nginx/conf.d/0000-' + upsteams.name + '-upstreams.conf', result, res);
                //     }
                // });
            } else {
                return res.json({
