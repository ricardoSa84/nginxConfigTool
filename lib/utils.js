        ...
        try {
            if (upsteams.instanceid === "localhost") {
                ...
                Instance.getVmId({_id : upsteams.instanceid}, res, function(result, res){
                    console.log("Resilt", result.details.interfaces[0]);
                });
                ...
                fs.writeFile('./temp/0000-' + upsteams.name + '-upstreams.conf', content, function(err) {
                    if (err) {
                        ...
                        sendRemote.writeFileRemote('./temp/0000-' + upsteams.name + '-upstreams.conf', '/etc/nginx/conf.d/0000-' + upsteams.name + '-upstreams.conf', result, res);
                    }
                });
