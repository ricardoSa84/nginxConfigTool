    ...
    processReceivedJson: function(ipInstance, json, result, local, res) {
        ...
        try {
            if (local === "local") {
                ...
                saveFile(ipInstance, './temp/1000-' + json.servername + '-' + json.port + '.conf', '/etc/nginx/conf.d/1000-' + json.servername + '-' + json.port + '.conf', content, "Server", local, result, res);
    ...
    processReceivedJsonUpstream: function(ipInstance, upsteams, result, local, res) {
        ...
        try {
            if (local === "local") {
                ...
                saveFile(ipInstance, './temp/0000-' + upsteams.name + '-upstreams.conf', '/etc/nginx/conf.d/0000-' + upsteams.name + '-upstreams.conf', content,"Upstream", local, result, res);
