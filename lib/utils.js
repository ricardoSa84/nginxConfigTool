...
module.exports = {
    ...
    processReceivedJsonUpstream: function(upsteams, result, res) {
        ...
        try {
            if (upsteams.instanceid === "localhost") {
                ...
                Instance.getVmId({ _id: upsteams.instanceid }, res, function(result, res) {
    ...
    removeFiles: function(result, upstreams, res) {
        try {
            console.log(upstreams);
            var upsteam = upstreams.split("-")
            if (upsteam[0] === "localhost") {
                fs.unlinkSync("/etc/nginx/conf.d/0000-" + upsteam[1] + "-upstreams.conf");
            } else {
                Instance.getVmId({ _id: upsteam[0] }, res, function(result, res) {
                    console.log(result);
                    var ipInstance = result.details.interfaces[0].ip;
