 ...
 */
 ServerSktIo.prototype.init = function () {
    ...
    this.server.io.on("connection", function (socket) {
        ...
        var logaccess = cp.exec('tail -f /var/log/nginx/access.log | grep -v /socket.io');
        var logerror = cp.exec('tail -f /var/log/nginx/error.log');
        ...
        logaccess.stdout.on("data", function (data) {
            socket.emit('logaccess', data.toString('utf-8'));
        }); 
        logerror.stdout.on("data", function (data) {
            socket.emit('logerror', data.toString('utf-8'));
        }); 
