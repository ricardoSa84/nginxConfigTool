...
var Collector = function(station) {
  this.requests = [];
  this.servers = [];
  this.serverstation = station;
...
Collector.prototype.isValid = function(req) {
  console.log("REQ", req);
  console.log(!req.remote_addr, req.remote_addr.indexOf(' ') !== -1 , !req.host, req.host.indexOf(' ') !== -1, !req.http_method , req.http_method.indexOf(' ') !== -1 , !req.status , !req.remote_user);
  console.log(req.remote_addr, req.remote_addr, req.host, req.host, req.http_method, req.http_method, req.status , req.remote_user);
...
Collector.prototype.loadDashode = function() {
  ...
  this.servers.push(new Server(os.hostname(), process.env.SERVER || self.serverstation || 'http://127.0.0.1:8080'));
