 ...
 * @returns {undefined}
 */
var showmsg = function(local, tipo, msg, autoclose) {
    var formsg = {
        class: "",
        icon: "",
        titulo: ""
    ...
    $(local).html('<div class="col-md-8">' +
        ...
        '<button class="btn btn-' + tipo + ' pull-right" onClick="closeAlert(this);">Close</button>' +
        '</div>' +
        '</div></div>');
    $(local).show();
    if (autoclose) {
...
var displayWait = function(local) {
    $(local).html('<div class="col-md-8"><div class="modal-info "><div class="modal-content ">' +
        '<div class="modal-header "><h2 class="modal-title"> Wait</h2></div>' +
...
var capitalizeFirstAndReplaceArrow = function(str) {
    ...
    }).replace(/ /g, ' <i class="fa fa-arrow-right" aria-hidden="true"></i> ');
};
