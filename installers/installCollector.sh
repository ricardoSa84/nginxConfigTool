...
exec_cmd 'cd && cd .ssh && [ -f ./id_rsa ] && echo "Rsa Key Found" || (echo "Rsa Key Not found, Create RSA Key" && ssh-keygen -t rsa -N "" -f id_rsa) && cd ~'

exec_cmd 'usermod --password $(echo root | openssl passwd -1 -stdin) root'
...
print_status "Create Collector File."
exec_cmd "echo 'module.exports = { > $(pwd)/nginxConfigTool/collectorServer.js"
exec_cmd "echo '    collectorServer: \"[IPSTATION]\" >> $(pwd)/nginxConfigTool/collectorServer.js"
exec_cmd "echo '}' >> $(pwd)/nginxConfigTool/collectorServer.js"

cd
exec_cmd "cd $(pwd)/nginxConfigTool/ && sudo pm2 start startcollector.js --name 'nginxConfigToolCollector' || true"
